...
object UrlUtil {
	 ...
	 */
	def getParentFriendlyURL(friendlyUrl: String): String = {
		if (friendlyUrl == null || friendlyUrl.equals("/")) ""
		val lastSlash = friendlyUrl.lastIndexOf('/')
		if (lastSlash == 0 || lastSlash == -1) "/"
		friendlyUrl.substring(0, lastSlash)
	}
	 ...
	 * @return
	 */
	def getNameFromFriendlyURL(friendlyUrl: String): String = {
		if (friendlyUrl == null || friendlyUrl.equals("/")) ""
		val lastSlash = friendlyUrl.lastIndexOf('/')
		if (lastSlash == -1) ""
		friendlyUrl.substring(lastSlash + 1, friendlyUrl.length)
	}
	
	/**
	 * Convert title to url. Replace spaces to _
	 */
	def titleToURL(title: String): String = title.replace(' ', '_').toLowerCase
